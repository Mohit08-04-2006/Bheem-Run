...
function setup(){
  ...
  gameover = createSprite(width/2,240,20,20);
  gameover.addImage(gameoverImg);
  
  restart = createSprite(width-630,290,20,20);
...
function draw(){
  ...
  // Jumping Bheem
  if (keyDown("space") || mousePressedOver(bheem) && bheem.collide(ground)){
    bheem.velocityY = -14;
  }
...
function spawnLadoos(){
  if(frameCount% 80 == 0){
  var s = Math.round(random(300,340));
